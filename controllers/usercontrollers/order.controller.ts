...
export const getOptimizedSplits = async (req: Request, res: Response) => {
  try {
    ...
    const serviceUrl = process.env.AI_FULFILLMENT_URL || "http://localhost:5001";
    console.log(`Connecting to AI Fulfillment Service at: ${serviceUrl}/api/ai/fulfillment/optimize`);
    
    const response = await axios.post(`${serviceUrl}/api/ai/fulfillment/optimize`, {
      prescriptionId: prescriptionId || `cart-${userId}-${Date.now()}`,
      patientId: userId,
      medicines,
      patientCoords: finalCoords,
    });
...
export const planFulfillmentPrescription = async (req: Request, res: Response) => {
  try {
    ...
    const serviceUrl = process.env.AI_FULFILLMENT_URL || "http://localhost:5001";
    console.log(`Connecting to AI Fulfillment Service Planner at: ${serviceUrl}/api/ai/fulfillment/planner`);

    const response = await axios.post(`${serviceUrl}/api/ai/fulfillment/planner`, {
      prescription,
      pharmacies,
    });
