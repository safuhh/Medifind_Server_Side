...
// consult time booking slot from user side
...
export const confirmPayment = async (req: Request, res: Response) => {
  try {
    ...
    // Verify payment status with Stripe (if not mock payment intent)
    if (!paymentIntentId.startsWith("mock_intent_")) {
      try {
        const paymentIntent = await stripe.paymentIntents.retrieve(paymentIntentId);
        if (paymentIntent.status !== "succeeded") {
          return res.status(400).json({ 
            success: false, 
            message: "Payment has not succeeded. Stripe status: " + paymentIntent.status 
          });
        }
      } catch (stripeError: any) {
...
export const getBookingById = async (req: any, res: Response) => {
  try {
    ...
    const booking = await DoctorBooking.findById(bookingId).populate("userId", "name email phone");
    if (!booking) {
