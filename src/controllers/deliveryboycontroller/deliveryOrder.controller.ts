    ...
    if (deliveryBoyLat !== undefined && deliveryBoyLng !== undefined) {
      filteredOrders = availableOrders.filter((order: any) => {
        ...
        if (order.items && order.items.length > 0) {
          for (const item of order.items) {
            ...
            if (sellerLoc?.coordinates && sellerLoc.coordinates.length === 2) {
              ...
              if (dist <= 15) { // 15km radius for pickup
                isWithinRange = true;
              }
            }
          }
        ...
        if (!isWithinRange && order.userId?.location?.coordinates && order.userId.location.coordinates.length === 2) {
          ...
          minDistance = Math.min(minDistance, dist);
          if (dist <= 25) { // 25km radius for dropoff
            isWithinRange = true;
          }
        }
        ...
        // 3. Fallback: if no coordinates exist at all, we don't show the order to anyone
        // because we can't calculate distance. The user/seller must have a valid location.
        if (!hasSellerCoords && !hasCustomerCoords) {
          isWithinRange = false;
        }

        order.__distance = minDistance;
        return isWithinRange; 
      });

      // Sort by distance ascending
      filteredOrders.sort((a: any, b: any) => (a.__distance || Infinity) - (b.__distance || Infinity));
    }
